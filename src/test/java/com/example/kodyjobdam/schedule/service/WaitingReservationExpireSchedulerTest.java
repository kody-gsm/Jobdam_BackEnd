...
class WaitingReservationExpireSchedulerTest {
    ...
    @BeforeEach
    void setUp() {
        // 한국 시간 00:05는 UTC로 아직 전날이다. 한국 날짜로 판단하는지 함께 확인한다.
        ZoneId zone = ZoneId.of("Asia/Seoul");
        ReflectionTestUtils.setField(scheduler, "clock",
                Clock.fixed(TODAY.atTime(0, 5).atZone(zone).toInstant(), zone));
    }

    @Test
    void 한국_날짜_기준으로_일반_진로_상담_대기_신청을_만료시킨다() {
        scheduler.expireWaitingReservations();

        verify(commonService).expireWaitingReservations(TODAY);
        verify(courseService).expireWaitingReservations(TODAY);
    }

    @Test
    void 일반_상담_만료가_실패해도_진로_상담은_만료시킨다() {
        when(commonService.expireWaitingReservations(TODAY)).thenThrow(new IllegalStateException("DB 오류"));
